...
<body>
    ...
    <?php
        ...
        function computePizzaTotal($pizzaPrice, $toppingPrices, $qty) {
            $toppingsTotal = 0;
            foreach ($toppingPrices as $price) {
                $toppingsTotal += $price;
            }
            $total = ($pizzaPrice + $toppingsTotal) * $qty;
            return $total;
        }
    ?>
                    ...
                    <div class="form-group">
                        ...
                        <?php foreach ($pizzaMenu as $name => $price): ?>
                            <div class="radio-item">
                                <input type="radio" name="pizza" value="<?= $name ?>" id="pizza_<?= $name ?>" required>
                                <label for="pizza_<?= $name ?>">
                                    <?= $name ?>
                        ...
                        <div class="checkbox-group">
                            <?php foreach ($toppingsMenu as $name => $price): ?>
                                <div class="checkbox-item">
                                    <input type="checkbox" name="toppings[]" value="<?= $name ?>" id="top_<?= $name ?>">
                                    <label for="top_<?= $name ?>">
                                        <?= $name ?>
                                        <span class="price">+₱<?= $price ?></span>
                                    </label>
            ...
            <div class="card">
                ...
                <?php
                    if (isset($_POST['order'])) {
                        ...
                        $pizzaPrice    = $pizzaMenu[$pizzaName];
                        $toppingPrices = [];
                        foreach ($toppings as $topping) {
                            $toppingPrices[$topping] = $toppingsMenu[$topping];
                        }

                        // TODO: 5. Call function and calculate Grand Total
                        $grandTotal = computePizzaTotal($pizzaPrice, $toppingPrices, $qty);

                        // TODO: 6. Display Results
                        echo "<div class='summary-item'><span>Customer</span><span><strong>$customer</strong></span></div>";
                        echo "<div class='summary-item'><span>Pizza</span><span>$pizzaName <span class='price'>₱$pizzaPrice</span></span></div>";

                        if (!empty($toppings)) {
                            foreach ($toppingPrices as $toppingName => $toppingPrice) {
                                echo "<div class='summary-item'><span>+ $toppingName</span><span class='price'>₱$toppingPrice</span></div>";
                            }
                        } else {
                            echo "<div class='summary-item'><span>Toppings</span><span>None</span></div>";
                        }

                        echo "<div class='summary-item'><span>Quantity</span><span>x$qty</span></div>";
            ...
            <div class="card full-width order-counter">
                ...
                <?php
                    if (isset($_POST['order'])) {
                        ...
                        for ($i = 1; $i <= $qty; $i++) {
                            echo "<div class='order-item'>Order #$i — $pizzaName pizza for $customer";
                            if (!empty($toppings)) {
                                echo " with " . implode(", ", $toppings);
                            }
                            echo "</div>";
                        }
                    }
